...
const TestResults = ({ route, navigation }) => {
  ...
  const getInterpretation = (value, type, referenceRange) => {
    if (!referenceRange) return { status: 'Bilinmiyor', color: '#666' };

    const range = referenceRange[type];
    if (!range) return { status: 'Bilinmiyor', color: '#666' };

    if (type === 'IgE') {
      if (value > range.max) return { status: 'Yüksek', color: '#f44336' };
      return { status: 'Normal', color: '#4caf50' };
    }

    if (value < range.min) return { status: 'Düşük', color: '#ff9800' };
    if (value > range.max) return { status: 'Yüksek', color: '#f44336' };
    return { status: 'Normal', color: '#4caf50' };
  };
